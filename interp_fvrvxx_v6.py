...
def compute_weight_matrix(Vra, Vxa, Vrb, Vxb, fV, Vr2pidVrb, dVxb, vraL, vraR, vxaL, vxaR):
    nvra, nvxa = Vra.size, Vxa.size
    nvrb, nvxb = Vrb.size, Vxb.size

    _weight = np.zeros((nvxa, nvra, nvxb, nvrb), dtype=np.float64)
    for ib in range(nvrb):
        for jb in range(nvxb):
            for ia in range(nvra):
                vraMin = max([fV * vraL[ia], vraL[ia]])
                vraMax = min([fV * vraR[ib], vraR[ia]])
                for ja in range(nvxa):
                    vxaMin = max([fV * vxaL[ja], vxaL[ja]])
                    vxaMax = min([fV * vxaR[jb], vxaR[ja]])
                    if vraMax > vraMin and vxaMax > vxaMin:
                        _weight[ja, ia, jb, ib] = 2 * np.pi * (vraMax ** 2 - vraMin ** 2) * (vxaMax - vxaMin) / (Vr2pidVrb[ib] * dVxb[jb])
    return np.reshape(_weight, (nvra * nvxa, nvrb * nvxb))
...
def interp_fvrvxx(fa, Vra, Vxa, Xa, Tnorma, Vrb, Vxb, Xb, Tnormb, do_warn=None, debug=0, correct=1, g=None):
    ...
    if w1_match or w2_match:
        ...
        weight = compute_weight_matrix(Vra, Vxa, Vrb, Vxb, fV, Vr2pidVrb, dVxb, vraL, vraR, vxaL, vxaR)
