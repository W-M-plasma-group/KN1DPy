...
def kinetic_h(vx,vr,x,Tnorm,mu,Ti,Te,n,vxi,fHBC,GammaxHBC,PipeDia,fH2,fSH,nHP,THP,fH,
			  ...
			  No_Recomb=0,H_H_EL=0,H_P_EL=0,_H_H2_EL=0,H_P_CX=0,ni_correct=0):
	...
	#	Set common block variables (Will fix later)
	vx_s=vr_s=x_s=Tnorm_s=mu_s=Ti_s=Te_s=n_s=vxi_s=fHBC_s=GammaxHBC_s=PipeDia_s=fH2_s=fSH_s=nHP_s=THP_s=fH_s=Simple_CX_s=JH_s=Collrad_s=Recomb_s=H_H_EL_s=H_P_EL_s=H_H2_EL_s=H_P_CX_s=None
	#	FS: added collrad_s
	vr2vx2=vr2vx_vxi2=fi_hat=ErelH_P=Ti_mu=ni=sigv=alpha_ion=v_v2=v_v=vr2_vx2=vx_vx=Vr2pidVrdVx=SIG_CX=SIG_H_H=SIG_H_H2=SIG_H_P=Alpha_CX=Alpha_H_H2=Alpha_H_P=MH_H_sum=Delta_nHs=Sn=Rec=None
	nH2=VxH2=TH2=None
	...
	while error==0:
		...
		if i_p.size==i_n.size:
			diff=[i for i in range(i_p.size) if i_p[i]!=-reverse(i_n)[i]]
		if diff.size>0:
	...
	if abs(ratio-1)>.01*ttruncate:
		fHBC=fHBC_input
	fH[0,ip,:]=fHBC_input[ip,:]
	...
	if Do_fH2_moments:
		...
		for k in range(nx):
			...
			if nH2[k]>0:
				...
				TH2[k]=(2*mu*mH)*Vth2*np.sum(Vr2pidVr*np.matmul(dVx,vr2vx2_ran2*fH2[k,:,:]))/(3*q*nH2[k])
	...
	if Do_sigv:
		...
		if Use_Collrad_Ionization:
			...
			if JH:
				sigv[1,:]=JHS_Coef(n,Te,no_null=True) # Johnson-Hinnov, limited Te range
			else:
	...
	if Do_v_v2==1:
		...
		for l in range(nvx):
			Vr2pidVrdVx[l,:,:,:]=Vr2pidVr[l]*dVx[l]
	...
	while do_fH_Iterate:
		...
		if ii.size<=0:
			...
			for k in range(nx-1,0,-1):
				fHG[k-1,i_n,:]=FHG[k,i_n,:]*Ck[k,i_n,:]+Gk[k,i_n,:]
				
			#	Compute first-flight neutral density profile

			for k in range(nx):
				NHG[igen,k]=np.sum(Vr2pidVr*np.matmul(dVx,gHG[k,:,:]))
	...
	if H_H_EL or H_P_EL or H_H2_EL:
		...
		for k in range(nx):
			...
			for i in range(nvr):
				vr2vx2_ran2[:,i]=vr[i]**2+(vx-VxHG[k]/Vth)**2
			THG[k]=mu*mH*Vth2*np.sum(Vr2pidVr*np.matmul(dVx,vr2vx2_ran2*fHG[k,:,:]))/(3*q*NHG[igen,k])
		if H_H_EL:
			if debrief>1:
				print(prompt+'Computing MH_H')
	...
	for k in range(nx):
		pH[k]=mu*mH*Vth2*np.sum(Vr2pidVr*np.matmul(dVx,vr2vx2_ran[k,:,:]*fH[k,:,:]))/(3*q)
	...
	for k in range(nx):
		piH_xx[k]=mu*mH*Vth2*np.sum(Vr2pidVr*np.matmul(dVx*(vx-_VxH[k])**2,fH[k,:,:]))/q-pH[k]

	#	piH_yy

	for k in range(nx):
		piH_yy[k]=mu*mH*Vth2*.5*np.sum(Vr2pidVr*vr**2*np.matmul(dVx,fH[k,:,:]))/q-pH[k]
	...
	for k in range(nx):
		qxH[k]=.5*mu*mH*Vth3*np.sum(Vr2pidVr*np.matmul(dVx*(vx-_VxH[k]),vr2vx2_ran[k,:,:]*fH[k,:,:]))
	...
	for k in range(nx):
		...
		QH[k]=.5*mu*mH*Vth2*np.sum(Vr2pidVr*np.matmul(dVx,vr2vx2_ran[k,:,:]*C))
		RxH[k]=mu*mH*Vth*np.sum(Vr2pidVr*np.matmul(dVx*(vx-_VxH),C))
		...
		RxW_H[k]=mu*mH*Vth*np.sum(Vr2pidVr*np.matmul(dVx*(vx-_VxH[k]),CW_H))
		EW_H[k]=.5*mu*mH*Vth2*np.sum(Vr2pidVr*np.matmul(dVx,vr2vx2[k,:,:]*CW_H))
		if H_H_EL:
			CH_H=Vth*Omega_H_H[k]*(MH_H_sum[k,:,:]-fH[k,:,:])
			for i in range(nvr):
				vr2_2vx_ran2[:,i]=vr[i]**2-2*(vx-_VxH[k])**2
			Epara_PerpH_H[k]=-.5*mu*mH*Vth2*np.sum(Vr2pidVr*np.matmul(dVx,vr2_2vx_ran2*CH_H))

	#	qxH_total

	qxH_total=(.5*nH*(mu*mH)*VxH*VxH+2.5*pH*q)*VxH+q*piH_xx*VxH+qxH

	#	QH_total

	QH_total=QH+RxH*VxH+.5*(mu*mH)*NetHSource*VxH*VxH
	...
	if Compute_Errors:
		...
		if CI_Test:
			...
			if H_P_EL:
				...
				for k in range(nx):
					RxCI_P_H[k]=-.5*(mu*mH)*Vth2*np.sum(Vr2pidVr*np.matmul(dVx,Alpha_H_P[k,:,:]*fH[k,:,:]))
			...
			if H_H2_EL:
				...
				for k in range(nx):
					RxCI_H2_H[k]=-2/3*(mu*mH)*Vth2*np.sum(Vr2pidVr*np.matmul(dVx,Alpha_H_H2[k,:,:]*fH[k,:,:]))
			...
			if H_H_EL:
				...
				for k in range(nx):
					Work[:]=fH[k,:,:]
					Alpha_H_H[:]=np.matmul(Work,SIG_H_H)
					Epara_Perp_CI[k]=.5*(mu*mH)*Vth3*np.sum(Vr2pidVr*np.matmul(dVx,Alpha_H_H*fH[k,:,:]))
		...
		for k in range(nx):
			qxH_total2[k]=.5*(mu*mH)*Vth3*np.sum(np.matmul(vx*dVx,Vr2pidVr[k,:,:]*fH[k,:,:]))
