...
for i=0,5 do begin
   ...
   sigv=SigmaV_EL_HH_P(Ti,E)
   if i eq 0 then begin
      plot,/nodata,Ti,sigv*1e6,/xlog,/ylog,xrange=[0.1,1e3],xstyle=1,$
           yrange=[1.0e-12,1.0e-8],ystyle=1,$
           title='H_2 + p -> H_2 + p elastic',xtitle='Ti (eV)',$
   ...
   dy=.03
   cs=0.8
   xyouts,.15,yleg,/normal,'output from IDL function SIGMAV_EL_HH_P.PRO which evaluates the reaction rates using',charsize=0.8
   ...
   xyouts,.15,yleg-4*dy,/normal,'S.V. Passovets, page 305. (Plenum Press, New York 1995)',charsize=cs
end
