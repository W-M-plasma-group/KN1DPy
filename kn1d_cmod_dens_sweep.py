...
for i in range(1, 6):
    ...
    '''plt.plot(xH2, nH2, linestyle = 'solid', color = 'red', label = f'nH2')
    #plt.plot(np.full(10, 0.17), np.linspace(np.min(nH2), np.max(nH2), 10), linestyle = ':', color = 'green', label = 'sep')
    plt.xlabel('Distance from Wall (m)')
    plt.ylabel('Density (m$^{-3}$)')
    plt.yscale("log")
    #plt.ylim(np.min(nH2), np.max(nH2))
    #plt.xlim(np.min(xH2), 0.175)
    plt.title(f'Python CMOD Density Profile for H$_2$')
    plt.legend()
    plt.savefig(f'/Users/Gwen/Desktop/KN1DPy-Nick/Plots/nH2_cmod_dens_py.png', dpi = 800)
    plt.show()
    ...
    plt.plot(xH, nH, linestyle = 'solid', color = 'blue', label = f'nH')
    #plt.plot(np.full(10, 0.17), np.linspace(np.min(nH), np.max(nH), 10), linestyle = ':', color = 'orange', label = 'sep')
    plt.xlabel('Distance from Wall (m)')
    plt.ylabel('Density (m$^{-3}$)')
    #plt.yscale("log")
    #plt.ylim(np.min(nH), np.max(nH))
    plt.xlim(np.min(xH), np.max(xH))
    plt.title(f'Python CMOD Density Profile for H')
    plt.legend()
    plt.savefig(f'/Users/Gwen/Desktop/KN1DPy-Nick/Plots/nH_cmod_dens_py.png', dpi = 800)
    plt.show()  
    exit()'''
...
for i in range(1, 6):
    plt.plot(xH2_tot[i - 1], nH2_tot[i - 1], linestyle = 'solid', label = f'nH2 by {i}')
...
plt.title(f'Python CMOD Density Profile for H$_2$ (dens amp)')
plt.legend()
plt.savefig(f'/Users/Gwen/Desktop/KN1DPy-Nick/Plots/nH2_cmod_den_sweep_py.png', dpi = 800)
plt.show()
print('Press any key to continue')
input()
print('continuing')
...
plt.figure(constrained_layout=True)
for i in range(1, 6):
    plt.plot(xH_tot[i - 1], nH_tot[i - 1], linestyle = 'solid', label = f'nH by {i}')
...
plt.title(f'Python CMOD Density Profile for H (dens amp)')
plt.legend()
plt.savefig(f'/Users/Gwen/Desktop/KN1DPy-Nick/Plots/nH_cmod_den_sweep_py.png', dpi = 800)
plt.show()      
