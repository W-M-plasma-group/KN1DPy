...
def create_kinetic_h_mesh(nv, mu, x, Ti, Te, n, PipeDia, E0 = 0, ixE0 = 0 ,irE0 = 0,fctr = 1):
    ...
    vx, vr, Tnorm = create_VrVxMesh(nv, Tifine)
    ...
    for k in range(0, nxfine):
        if PipeDiafine > 0:
            gamma_wall[k] = 2 * max(vr) * vth / PipeDiafine
    ...
    if Use_Collrad_Ionization:
        ...
        if JH:
            ioniz_rate = JHS_coef(nfine, Tefine, no_null = True)
        else:
            ioniz_rate = sigmav_ion_h0(Tefine)
    RR = nfine * ioniz_rate + nfine * sigma_cx_h0(Tifine, np.array([minE0] * np.size(minE0))) + gamma_wall
    ...
    while xpt > xminH:
        ...
        xpt = xpt - dxpt 
    xH = np.concatenate(xminH, xH[0:np.size(xH) - 2])
    ...
    vx, vr, Tnorm = create_VrVxMesh(nv, TiH)
