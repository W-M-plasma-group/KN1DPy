...
# set variables that we are borrowing from cmod 
LC = values_list[14]
PipeDia = values_list[15]

# initialize empty lists to save data from each run 
ne_KNEE_tot = [[], [], [], [], []]
nH2_tot = [[], [], [], [], []]
nH_tot = [[], [], [], [], []]
xH2_tot = [[], [], [], [], []]
...
# generate text file to save data
save_data = open("kn1d_sparc_dens_sweep_data.txt", "w")
save_data.write("Data for KN1DPy SPARC Density sweep(test): \n")
save_data.close()
...
for i in range(1, 6):
    ...
    # save the KNEE density
    ne_KNEE = ne[7]
    ne_KNEE_tot[i - 1] = ne_KNEE
    print(ne_KNEE_tot)
    ...
    # Below are the necessary inputs for KN1D 
    ...
    xH2, nH2, GammaxH2, TH2, qxH2_total, nHP, THP, SH, SP, \
                xH, nH, GammaxH, TH, qxH_total, NetHSource, \
                    ...
                    = KN1D( x, xlim, xsep, GuageH2, mu, Ti, Te, ne, vx, LC, PipeDia)
    # print results 
    print('xH2', xH2)
    print('nH2', nH2)
    ...
    save_data = open("kn1d_sparc_dens_sweep_data.txt", "a")
...
# plot the molecular hydrogen profiles
...
plt.title(f'Python SPARC Density Profile for H$_2$ (dens sweep)')
plt.legend()
plt.savefig(f'/Users/Gwen/Desktop/KN1DPy-Nick/Plots/nH2_sparc_dens_py.png', dpi = 800)
plt.show()
...
# plot the atomic hydrogen profiles 
plt.figure(constrained_layout=True)
for i in range(1, 6):
...
plt.title(f'Python SPARC Density Profile for H (dens sweep)')
plt.legend()
plt.savefig(f'/Users/Gwen/Desktop/KN1DPy-Nick/Plots/nH_sparc_dens_py_HR.png', dpi = 800)
plt.show() 

#plot the gradient data 
plt.figure(constrained_layout=True)
for i in range(0, 6):
    plt.plot(xH2_tot[i - 1], ngrad_H2[0][i - 1], marker = '.', label = f'input scaled by {i}')
plt.plot(np.full(10, 0.01), np.linspace(min(nH_mins), max(nH_maxs), 10), linestyle = ':', color = 'orange', label = 'sep')
plt.xlabel('Distance from Wall (m)')
plt.ylabel('Density (m$^{-3}$)')
plt.yscale("log")
plt.ylim(min(nH_mins), max(nH_maxs))
plt.xlim(min(xH_mins), max(xH_maxs))
plt.title(f'Python SPARC Gradients of Density Profile for H$_2$ (dens sweep)')
plt.savefig(f'/Users/Gwen/Desktop/KN1DPy-Nick/Plots/nH2_sparc_dens_grad_py_HR.png', dpi = 800)
plt.legend()
plt.show()